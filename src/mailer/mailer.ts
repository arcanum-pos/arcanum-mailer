...
// original license and exactly what changed and why. Two changes from
// upstream:
//
// 1. EHLO/HELO hostname was hardcoded to the literal string "127.0.0.1"
//    (see upstream issue #55, confirmed and root-caused there) — some
//    strict receiving mail servers silently reject/drop mail whose HELO
//    argument is a bare loopback IP (works fine sending to lenient
//    providers like Gmail, silently fails elsewhere — no error, message
//    just never arrives). Now takes a required `clientName` option
//    instead; questo-mail's own send.ts derives it from the configured
//    From address's domain.
// 2. Dropped CRAM-MD5 auth (its only use was HMAC-MD5 via node:crypto,
//    which needed the nodejs_compat flag for this one legacy mechanism).
//    Not needed: Gmail and every other provider this app targets support
//    AUTH LOGIN/PLAIN over TLS, which an app password already works with.
//    Removing it means this Worker needs no compatibility flags at all.
...
export type WorkerMailerOptions = {
  ...
  // Sent as the EHLO/HELO argument — should be a real hostname/domain, not
  // an IP literal (see the file-level comment above). questo-mail derives
  // this from the configured From address's domain.
  clientName: string
  logLevel?: LogLevel
  dsn?:
    | {
...
export class WorkerMailer {
  ...
  private async ehlo() {
    await this.writeLine(`EHLO ${this.clientName}`)
    const response = await this.readTimeout()
  ...
  private async helo() {
    await this.writeLine(`HELO ${this.clientName}`)
    const response = await this.readTimeout()
    if (response.startsWith('2')) {
      return
    }
